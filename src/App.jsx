...
function App() {
  ...
  if (view === 'login') {
    return (
      <div className="h-screen w-screen flex items-center justify-center bg-gradient-to-br from-red-900 via-red-800 to-slate-900 overflow-hidden relative">
        ...
        <div className="z-10 bg-white/10 backdrop-blur-lg border border-white/20 p-8 rounded-2xl w-full max-w-sm shadow-2xl m-4 text-center relative">
          <div className="mb-8 flex flex-col items-center justify-center gap-2"><img src={logoQcst} alt="QCST Logo" className="h-16 w-auto object-contain drop-shadow-lg animate-fade-in-down" /><img src={logoAtlasWhite} alt="ATLAS Logo" className="h-40 w-auto object-contain drop-shadow-xl animate-fade-in-up" /><p className="text-red-50 text-xs font-medium tracking-wider uppercase border-t border-red-400/30 pt-4 mt-2 w-full">Aplikasi Terpadu Layanan Analisa Sample</p></div>
          <form onSubmit={handleLogin} className="space-y-4" autoComplete="off">
            <input type="text" placeholder="Username" className="w-full bg-red-950/40 border border-red-300/30 rounded-lg p-3 text-white placeholder-red-200 focus:ring-2 focus:ring-red-500 focus:border-transparent outline-none transition-all" value={loginForm.username} onChange={e => setLoginForm({...loginForm, username: e.target.value})} autoComplete="off" />
            <div className="relative"><input type={showPassword ? "text" : "password"} placeholder="Password" className="w-full bg-red-950/40 border border-red-300/30 rounded-lg p-3 pr-12 text-white placeholder-red-200 focus:ring-2 focus:ring-red-500 focus:border-transparent outline-none transition-all" value={loginForm.password} onChange={e => setLoginForm({...loginForm, password: e.target.value})} autoComplete="new-password" /><button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute right-3 top-1/2 transform -translate-y-1/2 text-red-200 hover:text-white transition-colors focus:outline-none">{showPassword ? <EyeOff size={20} /> : <Eye size={20} />}</button></div>
            <div className="flex items-center gap-2 text-red-100/80 hover:text-white transition-colors cursor-pointer" onClick={() => setLoginForm({...loginForm, remember: !loginForm.remember})}><div className={`w-5 h-5 border-2 rounded flex items-center justify-center transition-all ${loginForm.remember ? 'bg-red-600 border-red-600' : 'border-red-300/50'}`}>{loginForm.remember && <CheckCircle size={14} className="text-white" />}</div><span className="text-xs font-bold tracking-wide select-none">Ingat Saya (Auto Login)</span></div>
            <button disabled={loading} className="w-full bg-gradient-to-r from-red-700 to-red-600 hover:from-red-600 hover:to-red-500 text-white py-3 rounded-lg font-bold transition-all shadow-lg shadow-red-900/50 tracking-wider">{loading ? 'MEMPROSES...' : 'MASUK SISTEM'}</button>
          </form>
          <p className="mt-8 text-center text-[10px] text-red-200/70 uppercase tracking-widest">© 2025 PT Semen Tonasa | Quality Control</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="flex h-screen w-screen bg-slate-50 overflow-hidden font-sans text-slate-800 relative">
      ...
      <aside className={`fixed inset-y-0 left-0 z-50 bg-gradient-to-b from-red-900 to-slate-900 text-white transform transition-all duration-300 ease-in-out shadow-2xl flex flex-col ${isMobileMenuOpen ? 'translate-x-0' : '-translate-x-full'} md:translate-x-0 md:relative ${isSidebarCollapsed ? 'md:w-20' : 'md:w-64'}`}>
        <div className={`p-6 border-b border-red-800/50 flex items-center ${isSidebarCollapsed ? 'justify-center p-4' : 'justify-between'}`}> {!isSidebarCollapsed && ( <div><img src={logoAtlasWhite} alt="ATLAS" className="h-8 w-auto mb-2" /><p className="text-[10px] text-red-200 uppercase tracking-widest">Lab Management System</p></div> )} <button onClick={() => setIsSidebarCollapsed(!isSidebarCollapsed)} className="hidden md:block text-red-200 hover:text-white transition-colors"><Menu size={24} /></button><button onClick={() => setIsMobileMenuOpen(false)} className="md:hidden text-red-200 hover:text-white"><X/></button></div>
        <nav className="flex-1 p-3 space-y-2 overflow-y-auto"> 
            ...
            {session.role !== 'analyst' && session.role !== 'manager' && (
                <>
                    <SidebarItem icon={<FlaskConical/>} label="Request Uji" active={view === 'order'} collapsed={isSidebarCollapsed} onClick={() => {setView('order'); setIsMobileMenuOpen(false)}} />
      ...
      <div className="flex-1 flex flex-col h-full overflow-hidden bg-slate-100">
        ...
        <main className="flex-1 overflow-y-auto p-4 md:p-8 relative">
          ...
          <div className="relative z-10">
            {view === 'dashboard' && ( <div className="max-w-6xl mx-auto space-y-6 animate-fade-in"> <div className="bg-gradient-to-r from-red-800 to-red-600 rounded-2xl p-6 md:p-10 text-white shadow-lg relative overflow-hidden border-b-4 border-red-900"> <div className="relative z-10"> <h2 className="text-2xl md:text-3xl font-bold mb-2 tracking-tight">{greeting}, {session.full_name}</h2> <p className="text-red-100 mb-8 max-w-lg text-sm md:text-base leading-relaxed">Sistem ATLAS siap membantu proses analisa sampel Anda hari ini dengan standar kualitas terbaik.</p> {(session.role !== 'analyst' && session.role !== 'manager') && <button onClick={() => setView('order')} className="bg-white text-red-800 px-6 py-3 rounded-lg font-bold shadow-md hover:shadow-lg hover:bg-red-50 transition-all flex items-center gap-2 text-sm tracking-wide"><Plus size={18}/> BUAT PERMINTAAN BARU</button>} </div> <FlaskConical className="absolute -bottom-12 -right-12 text-white/10 w-72 h-72 rotate-12" /> </div> <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4"> <StatCard label="Total Sampel Masuk" value={stats.totalSamples} color="red" /> <StatCard label="Menunggu Proses" value={stats.pendingCount} color="purple" /> <StatCard label="Sedang Diproses" value={stats.processingCount} color="yellow" /> <StatCard label="Selesai" value={stats.completedCount} color="green" /> <StatCard label="Total Tiket" value={stats.totalTickets} color="blue" /> </div> <div className="grid md:grid-cols-2 gap-6 mt-6"> <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200"><h3 className="font-bold text-slate-800 mb-6 flex items-center gap-2"><TrendingUp className="text-red-600"/> Sampel Terbanyak Diuji (Global)</h3><div className="h-80 md:h-64 w-full"><ResponsiveContainer width="100%" height="100%"><BarChart data={stats.materialDistribution} layout="vertical" margin={{top: 5, right: 30, left: 10, bottom: 5}}><CartesianGrid strokeDasharray="3 3" horizontal={false} /><XAxis type="number" hide /><YAxis type="category" dataKey="name" width={80} tick={{fontSize: 11, fontWeight: 'bold'}} /><Tooltip cursor={{fill: '#f3f4f6'}} /><Bar dataKey="value" fill="#b91c1c" radius={[0, 4, 4, 0]} barSize={24} /></BarChart></ResponsiveContainer></div></div> <div className="space-y-6"> <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200 flex flex-col items-center"> <h3 className="font-bold text-slate-800 mb-2 w-full text-left">Status Pengerjaan Lab</h3> <div className="h-64 md:h-48 w-full"><ResponsiveContainer width="100%" height="100%"><PieChart><Pie data={stats.statusDistribution} cx="50%" cy="50%" innerRadius={45} outerRadius={70} paddingAngle={4} dataKey="value">{stats.statusDistribution.map((entry, index) => (<Cell key={`cell-${index}`} fill={STATUS_COLORS[entry.name] || '#cbd5e1'} stroke="none" />))}</Pie><Tooltip contentStyle={{ borderRadius: '8px', border: 'none', boxShadow: '0 4px 12px rgba(0,0,0,0.1)' }} itemStyle={{ color: '#1e293b', fontWeight: 'bold', fontSize: '12px' }} /><Legend layout="vertical" verticalAlign="middle" align="right" iconType="circle" iconSize={10} wrapperStyle={{ fontSize: "11px", fontWeight: "600", color: "#475569" }} /></PieChart></ResponsiveContainer></div> </div> <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200 flex-1"><h3 className="font-bold text-slate-800 mb-4 flex items-center gap-2"><Award className="text-yellow-500"/> Top Kontributor</h3><ul className="space-y-3">{stats.topUsers.map((u, idx) => (<li key={idx} className="flex justify-between items-center text-sm border-b border-slate-50 pb-2 last:border-0"><span className="flex items-center gap-2"><span className={`w-5 h-5 rounded-full flex items-center justify-center text-[10px] font-bold ${idx===0?'bg-yellow-100 text-yellow-700':idx===1?'bg-gray-100 text-gray-700':'bg-orange-50 text-orange-700'}`}>{idx+1}</span>{u.name}</span><span className="font-bold text-slate-600">{u.count} Tiket</span></li>))}</ul></div> </div> </div> </div> )}
