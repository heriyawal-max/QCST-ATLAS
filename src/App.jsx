...
const sendTelegramNotification = async (ticketNumber, fileName, pdfUrl) => {
  ...
  // Kita arahkan ke halaman detail tiket dengan parameter approval
  const approveLink = `${appLink}/?action=approve&ticket=${ticketNumber}`;
  const rejectLink = `${appLink}/?action=reject&ticket=${ticketNumber}`;
...
👇 *Silakan klik tombol di bawah untuk Validasi:*
    `;
...
function App() {
  ...
  // 👇 LOGIC BARU: HANDLE ACTION DARI URL (TELEGRAM)
  // [DIHAPUS] Karena sekarang pakai Webhook Backend (api/telegram.js)
  // useEffect(() => { ... }, [session]);
                            ...
                            >
                              ...
                              {(session.role === "admin" ||
                                ...
                                session.role === "analyst") && (
                                  <label
                                    className={`flex items-center gap-2 bg-slate-800 text-white px-4 py-2 rounded-lg text-xs font-bold hover:bg-slate-700 shadow-sm cursor-pointer transition-all ${uploadingId === req.id ? "opacity-50 pointer-events-none" : ""
                                      }`}
                                  >
                                    {uploadingId === req.id ? (
